...
export default class TableList extends PureComponent {
  ...
  handleCreate = () => {
    const { dispatch } = this.props;
    dispatch(routerRedux.push('/data-source/add-api'));
  };

  handelEdit = id => {
    const { dispatch } = this.props;
    dispatch(routerRedux.push(`/data-source/edit-api/${id}`));
  };
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          ...
          <Col md={8} sm={24}>
            <FormItem label="API地址">{getFieldDecorator('apiUrl')(<Input />)}</FormItem>
          </Col>
          <Col md={8} sm={24}>
            <FormItem label="时间">
              {getFieldDecorator('dataRange')(<RangePicker style={{ width: '100%' }} />)}
            </FormItem>
          </Col>
        </Row>
